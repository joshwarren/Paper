...
%Joshua Warren,$^{1}$\thanks{Contact e-mail: \href{mailto:joshua.warren@physics.ox.ac.uk}{joshua.warren@physics.ox.ac.uk}}
  Martin Bureau,$^{1,2}$ Bernd Hasemann,$^{3}$ Ilaria Ruffa,$^{4}$ Isabella
  Prandoni,$^{4}$ \newauthor Francesco Santoro,$^{4}$ Robert
  Laing,$^{4}$ Paola Parma,$^{4}$ Rosita Paladino,$^{4}$ Hans de Ruiter,$^{4}$ \newauthor
  Arturo Mignano$^{4}$ and Timothy Davis$^{5}$
  \\
...
\begin{table*}[t]
  \begin{center}
    ...
    \parbox[t]{0.7\textwidth}{\footnotesize\textit{Notes:} Col.~1:
      ...
      galaxy by its PKS name only. Col.~3: redshift (uncertainty in redshift is $< 0.00001$). Col.~4: National
...
spectroscopic redshift and is listed in Table~\ref{tab:sample}. 
After this step, each bin is analysed
independently.
...
quality of the fits to the stellar 
spectrum for all bins. For this reason, when fitting the stellar spectra
only (i.e.\ Sections~\ref{subsec:starKin}), the wavelength range of the NGC~1316 spectra was
restricted to $<5800$~\AA. The best-fitting stellar spectra were then extrapolated above 5800~\AA.
...
doublets: 
[\ion{O}{II}]$\lambda\lambda$3726,3729,
...
[\ion{S}{ii}]$\lambda\lambda$6716,6731. To minimise template 
...
data, before the line strengths are measured. These are then corrected for the effects of the stellar velocity 
...
the best-fitting luminosity-weighted single stellar population (SSP) model from an
...
In reality, the field of view does not always fully contain 
a given fitted ellipse. When this 
is the case, we follow the method of \citet{Emsellem2007}, where 
$R_\text{m}$ is redefined as $R_\text{m}\equiv\sqrt{A_\text{s}/\pi}$, 
where $A_\text{s}$ is the area contained within 
the field of view of the given ellipse. {\it For a given galaxy, we fit ellipses 
as described above, up to $R_\text{m}=R_\text{max}$, where 
$R_\text{max}$ is defined such that  
$\frac{A_\text{s}(R_\text{max})}{A_\text{ellipse}(R_\text{max})} 
\equiv 0.15$ or $R_\text{max} = R_\text{e}$, which ever is smallest.
} The value $\epsilon_\text{e}$, the ellipticity of this largest 
fitted ellipse, is given in Column~3 of 
...
measured eastward from North. A global value of
...
where$PA_\text{phot}$ is measured at $R_\text{max}$.
...
sum is computed over all $N$ bins within $1$~$R_\text{max}$ of the
...
regular rotators. This compares to $82\pm2\%$ of the Atlas$^\text{3D}$
galaxies being classified as regular rotators
\citep{Krajnovic2011}. However, the likelihood of a given galaxy
...
$13\pm2\%$ and $80\pm6\%$ of their sample galaxies to be slow
rotators. As in the regular-/non-regular-rotator classification scheme
(Section~\ref{subsec:RR-NRR}), slow rotators are more likely to be
found in higher mass galaxies (see lower panel of Fig.~\ref{fig:SRmassFraction}), and the three samples
...
\begin{figure}
  ...
  \caption{Upper panel: mass distribution of our Southern Sample (red,
    ...
    total number of galaxies in each bin.}
...
metallicity. (Mg$_2$ is considered to be a molecular index and is thus measured 
in units of mag$^{-1}$.) We can thus use Mg~$b$ as a proxy for
Mg$_2$.
...
$3.3\pm1.0$~\AA~dex$^{-1}$. We noted that NGC~1316 is again ana possible
outlier. Excluding NGC~1316 from the fit
...
Southern Sample. This was used over the larger Atlas$^\text{3D}$ sample \citep{Cappellari2011}, as stellar populations were not published by the Atlas$^\text{3D}$ team.
...
For many indices \citet{Ogando2008} found significantly stronger measurements to us.
...
Section~\ref{subsec:absorption} to produce maps of the most-likely luminosity-weighted SSP
parameters (see Appendix~\ref{sec:maps}): age ($t$), metallicity
($[M/H]$) and alpha-element enhancement ($\alpha$/Fe).

In general, our Southern Sample galaxies show old, metal-rich and
alpha-enhanced SSPs, characteristics that are common in ETGs \citep[e.g.][]{Worthey1992} The exceptions are NGC~612 and
...
$\mathrm{[M/H]}=0.12\pm0.01$ using a long slit with width $1\arcsec$ and length $0.1$~kpc).
 We also find NGC~612
...
diameter, and NGC~3100 has a branch of young stars ($t\approx4$\,Gyr) with very high metallicity projecting southwards from below the eastern branch of the molecular gas (cyan contours in
Fig.~\ref{fig:ngc3100}).
...
[M/H] and $\log(R/R_\text{e})$, i.e.\ as is common the gradients
...
\begin{table*}
  \begin{center}
    ...
    \begin{tabular*}{0.95\textwidth}{@{\extracolsep{\fill}}l c c c c c c c c}
      \hline
      \hline 
      Galaxy & \multicolumn{2}{c}{$\Delta\log(t)$} & \multicolumn{2}{c}{$\Delta\text{[M/H]}$} & \multicolumn{2}{c}{$\log(M_\text{\ion{H}{ii}}/\mathrm{M_\odot})$} & Balmer Dec. & Ionisation\\
...
$\Delta\text{[M/H]}=-0.03\pm0.01$. These are consistent with the
...
\citet{McDermid2006} showed that 
...
population. Using the method of \citet{McDermid2006}, the size 
and age of the KDC is defined as the radius
where the superposition of the two kinematic components (the KDC and
the host galaxy) results in a local minimum in the mean velocity and the
age of the most-likely SSP model for the spatially integrated spectrum
within a central, circular aperture with a radius of 
$1\arcsec$, respectively.
...
\begin{figure}
  ...
  \caption[KDC dichotomy]{KDC size\,--\,age relation. KDCs can be
    ...
    and grey for respectively fast and slow rotators.}
  \label{fig:KDC}
\end{figure}
...
1--2 spaxels. This means no structure can be
...
lines have a similar distribution (but different fluxes). 

As can be seen from the maps in Appendix~\ref{sec:maps}, 
both NGC~612 and NGC~3100 have ionised gas emission offset from the center 
of the galaxy. In NGC~612, the ionised gas does not appear related to either 
the radio jet (extending $\approx200$~kpc to the East and West of the galaxy 
...
Table~\ref{tab:derivedProp}. They are at the upper 
range of, and occasionally exceed, the 
...
adds to the evidence that NGC~612 has not undergone a
...
maps in cyan; see Fig.~\ref{fig:ngc3100}) show a
broken ring, with the radio jet passing through the gaps. This suggests 
...
nearly totally blueshifted). What is clear, is that there is a complete lack of any large-scale rotation of the ISM.
...
or stars on the post-asymptotic giant branch, pAGB),
...
(MEx) plot of \cite{Nyland2016}, using parameters derived from a 
spectra produced by spatially-integrating over a central circular apertures of radius
$3\arcsec$ to probe the central 
regions of the galaxies (see
Section~\ref{subsubsec:MEx}). This allows us to classify the remaining galaxies (with
...
not (simply labelled as LINER: they may be AGN or LIER). As the least
...
radiation from pAGB stars, as the dominant cause of the
...
\begin{figure}
  ...
  \caption{H$\alpha$ flux radial profiles derived from the MUSE
    ...
    PSF. The fitting of these lines was done by eye.
...
\begin{figure}
  ...
  \caption{As Fig.~\ref{fig:Ha_profile_MUSE}, but for the H$\beta$
    flux radial profiles derived from the VIMOS data.
...
suggesting that stellar processes also have some impact. Indeed 
\citet{Asabere2016} found NGC~612 to have a star formation rate an 
order of magnitude high than that of NGC~1316.
...
diagnostic plot is shown in Fig.~\ref{fig:MEx}. In particular, this 
...
\begin{figure}
  ...
  \caption{Nuclear MEx plot, allowing to classify the sources of
    ...
    line. Classification boundaries are from \citet{Nyland2016}.}
  \label{fig:MEx}
\end{figure}
...
Southern Sample have typical ETG properties, with old, metal-rich, alpha-enhanced
stellar populations. The few exceptions are NGC~612
...
the KDC age\,--\,size relation found by \citet{McDermid2006} in optically-selected ETGs (see 
Section~\ref{subsec:popKDC}). 
...
star formation in any of our targets. We conclude, therefore,
...
and occasionally exceed, the typical masses observed in ETGs,
...
\citep[including the SAURON Colourmaps; ][]{Cappellari2003}.
...
If we assume that the different ionisation states of a
given atomic species, observed along the same line-of-sight, occur in 
the same spatial location (and therefore are produced under the same 
conditions) it is natural to expect a simple relationship
...
\begin{figure}
  ...
  \bracket{\ion{N}{i}}/H$\beta$]{Top: [\ion{N}{ii}]/H$\alpha$ vs.\
    ...
    best-fit)/[\ion{N}{i}]/H$\beta$. The solid lines show the best-fitting
    Eq.~\ref{eq:N_dec}, while the dashed and dotted lines show
    respectively the $68$ and $99\%$ probability contours.}
...
\begin{figure*}
  ...
  \caption{Spatially-resolved map of all the parameters derived from
    ...
    available \citep[see][]{ruffa2019a, ruffa2019b}.}
